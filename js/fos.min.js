...
class MenuItem extends HTMLElement {
  ...
  render() {
    this.shadow.innerHTML =
      "<style>:host{display: block;background-color: inherit;}div:hover{color: gray;}</style><div><slot></slot></div>";
  }
}
...
class Window extends HTMLElement {
  ...
  connectedCallback() {
    ...
    ((this.top = (0.2 * innerHeight * howMany) / 5),
      (this.left = (0.1 * innerWidth * howMany) / 5),
      this.render());
  }
  render() {
    this.shadow.innerHTML = "";
...
class Desktop extends HTMLElement {
    ...
    if (this.selected && !this.selected.fixed)
      if ((e.preventDefault(), "fos-icon" === this.selected.localName)) {
        ...
        ((this.selected.style.left =
          8 * Math.floor(x / 8) - this.selected.offsetWidth / 2 + "px"),
          (this.selected.style.top =
            8 * Math.floor(y / 8) - this.selected.offsetHeight / 2 + "px"));
      } else {
        let newX = e.pageX || (e.touches ? e.touches[0].pageX : 0),
          newY = e.pageY || (e.touches ? e.touches[0].pageY : 0);
        ((this.selected.style.left =
          this.selected.offsetLeft + (newX - this.lastX) + "px"),
          (this.selected.style.top =
            this.selected.offsetTop + (newY - this.lastY) + "px"),
          (this.lastX = newX),
          (this.lastY = newY));
      }
  }
  ...
  get iconOffset() {
    return this.hasAttribute("iconOffset")
      ? this.getAttribute("iconOffset")
      : null;
  }
  set iconOffset(val) {
    val
  ...
  render() {
    this.shadow.innerHTML =
      "<style>:host{position: relative;display: block;width: 100%;height: 100%;min-height: 240px;min-width: 320px;background-color: #666;overflow: auto;}</style><slot></slot>";
  }
}
...
class Menu extends HTMLElement {
  ...
  render() {
    this.shadow.innerHTML =
      ...
      ";position: absolute;" +
      (this.bar ? this.bar.position : "bottom") +
      ": 40px;background-color: #AAA;padding: 1em;border: solid 1px black;border-" +
      (this.bar ? this.bar.position : "bottom") +
      `: 0;}#title:hover{color: gray;}</style><div><div id="title" part="title">${this.name}</div><div id="menu" part="window"><slot></slot></div></div>`;
  }
}
...
class Panel extends HTMLElement {
  ...
  bringFront() {
    ...
    for (const panel of panels)
      ((panel.style.display = "none"), (panel.selected = !1));
